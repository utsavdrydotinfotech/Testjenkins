...
var googleAuthUrl = 'https://www.googleapis.com/oauth2/v3/tokeninfo?access_token=';
...
//jwt token generation function for apple signin
function applesignIn(user, accessKey, webAccessKey) {
    var token = jsonwebtoken.sign({
        id: user[0].userId,
        name: user[0].username,
        deviceType: user[0].deviceType,
    ...
    async.waterfall([
        ...
        function register(userData, callback) {
            ...
            var cypher = `MERGE (a: User {username : "` + userData.username + `",  phoneNumber : "` + userData.phonenumber + `", ` +
                ...
                `a.userType ="` + userData.userType + `", a.shopName = "` + userData.shopName + `", ` +
                ...
                `RETURN DISTINCT a.username AS username, a.phoneNumber AS phoneNumber, a.pushToken AS pushToken, a.fullName AS fullName, a.userType AS userType, a.shopName AS shopName, a.profilePicUrl AS profilePicUrl, ` +
...
/**
 * 
 * function to register a user through apple account
 */
authServices.appleRegistration = (userData, cb) => {
    var userCollection = mongoDb.collection('user');
    ...
    async.waterfall([
        ...
        function usernameCheck(callback) {
            ...
            dbneo4j.cypher({
                ...
                if (e) {
                    ...
                    console.log('1212121233333333333333333333333')
                    userCollection.count({
                        username: userData.username
                    }, (mError, mData) => {
                        if (mError) {
        ...
        function checkAppleId(userData, callback) {
            console.log('check if appleId taken---------------------', userData.appleId);
            let query = `MATCH (a : User {appleId : "` + userData.appleId + `"}) RETURN COUNT(a) AS user; `;
            // callback(query, null);
            dbneo4j.cypher({
                query: query
            }, (e, d) => {
                if (e) {
                    responseObj = {
                        code: 500,
                        message: 'internal server error while checking if appleId is already taken or not',
                        error: e
                    };
                    callback(responseObj, null);
                } else if (d[0].user > 0) {
                    responseObj = {
                        code: 409,
                        message: 'already registered with this appleId'
                    };
                    callback(responseObj, null);
                } else {
                    callback(null, userData);
        ...
        function register(userData, callback) {
            delete userData.googleToken;
            // callback(null, userData);
            // userData.profilePicUrl = 'rtrtrt'
            console.log('reached till hererer-=-=-=-=-=-=-=', userData.username)
            /* console.log('reached till hererer-=-=-=-=-=-=-=',userData.phonenumber)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.appleId)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.createdOn)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.pushToken)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.email)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.deviceType)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.password)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.profilePicUrl)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.fullName)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.location)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.latitude)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.longitude)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.countrySname)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.city)
            console.log('reached till hererer-=-=-=-=-=-=-=',userData.createdOn) */

            /*            var cypher = `MERGE (a: User {username : "` + userData.username + `",  phoneNumber : "` + userData.phonenumber + `", ` +
                           `appleId : "` + userData.appleId + `" }) SET a.createdOn = "` + userData.createdOn + `", a.pushToken = "` + userData.pushToken + `", ` +
                           `a.deviceType = "` + userData.deviceType + `", a.password = "` + userData.password + `", a.profilePicUrl = "` + userData.profilePicUrl + `", ` +
                           `a.appleVerfied = 1, a.email = "` + userData.email + `", a.fullName = "` + userData.fullName + `", ` +
                           `a.location = "` + userData.location + `", a.latitude = "` + userData.latitude + `", a.longitude = "` + userData.longitude + `", ` +
                           `a.countrySname = "` + userData.countrySname + `", a.city = "` + userData.city + `" ` +
                           `CREATE UNIQUE (a)-[f : FOLLOWS {startedFollowingOn:` + userData.createdOn + `, followRequestStatus : ` + 1 + `}]->(a) ` +
            ...
            var cypher = `MERGE (a: User {username : "` + userData.username + `"` +
                `,appleId : "` + userData.appleId + `" }) SET a.createdOn = "` + userData.createdOn + `", a.pushToken = "` + userData.pushToken + `", ` +
                `a.deviceType = "` + userData.deviceType + `", a.password = "` + userData.password + `", a.profilePicUrl = "` + userData.profilePicUrl + `", ` +
                `a.appleVerfied = 1` + `, a.fullName = "` + userData.fullName + `", ` +
                `a.userType ="` + userData.userType + `", a.shopName = "` + userData.shopName + `", ` +
                `a.location = "` + userData.location + `", a.latitude = "` + userData.latitude + `", a.longitude = "` + userData.longitude + `", ` +
                `a.city = "` + userData.city + `" ` +
                `CREATE UNIQUE (a)-[f : FOLLOWS {startedFollowingOn:` + userData.createdOn + `, followRequestStatus : ` + 1 + `}]->(a) ` +
                `RETURN DISTINCT a.username AS username, a.phoneNumber AS phoneNumber, a.pushToken AS pushToken, a.profilePicUrl AS profilePicUrl, ` +
                `a.fullName AS fullName, a.userType AS userType, a.shopName AS shopName,` +
                `a.deviceType AS deviceType, toInt(a.createdOn) AS createdOn, a.email AS email, ` +
                `a.googleId AS googleId, a.facebookId AS facebookId, ID(a) AS userId LIMIT 1;`;

            console.log('to insert--=2222222222222-=-=-cypher', cypher);
            ...
            dbneo4j.cypher({
                ...
                if (e) {
                    ...
                    var authToken = applesignIn(d, accessKey, webAccessKey);

                    console.log('reg----------', authToken);
                    ...
                    var query = userCollection.insert({
                        ...
                        console.log('inbserted in mongodb???????????????', result);
                        responseObj = {
                            code: 200,
                            message: 'success',
                            response: {
...
authServices.register = (userData, cb) => {
    ...
    async.series([
        function checkUsername(callback) {
            ...
            // let query = `MATCH (a : User {username : "` + userData.username + `", userType : "` + userData.userType + `"}) RETURN DISTINCT COUNT(a) AS user; `;
            // cb(query, null);
            dbneo4j.cypher({
                query: query
            }, (e, d) => {
        ...
        function register(callback) {
            var cypher2 = userData.userType && userData.userType == "seller" ? `a.userType ="` + userData.userType + `", a.shopName = "` + userData.shopName + `", a.gstNumber = "` + userData.gstNumber + `",  a.contactName = "` + userData.contactName + `", ` : ``
            var cypher3 = userData.userType && userData.userType == "seller" ? `a.userType AS userType,  a.shopName AS shopName, a.gstNumber AS gstNumber, a.contactName AS contactName, ` : ``
            ...
            var cypher = `MERGE (a: User {username : "` + userData.username + `",  phoneNumber : "` + userData.phonenumber + `"}) ` +
                ...
                `a.location ="` + userData.location + `", a.latitude = "` + userData.latitude + `", a.longitude = "` + userData.longitude + `", ` + cypher2 +
                `a.countrSname= "` + userData.countrySname + `", a.city = "` + userData.city + `" ` +
                `CREATE UNIQUE (a)-[f : FOLLOWS {startedFollowingOn:` + userData.createdOn + `, followRequestStatus : ` + 1 + `}]->(a) ` +
                `RETURN DISTINCT a.username AS username, a.phoneNumber AS phoneNumber, a.pushToken AS pushToken, ` +
                `a.fullName AS fullName, a.profilePicUrl AS profilePicUrl, ` + cypher3 +
    ...
    async.waterfall([
        ...
        function register(facebookData, callback) {
            var cypher = `MERGE (a: User {username : "` + userData.username + `",  facebookId : "` + facebookData.id + `"}) ` +
                ...
                `CREATE UNIQUE (a)-[f : FOLLOWS {startedFollowingOn:` + userData.createdOn + `, followRequestStatus : ` + 1 + `}]->(a) ` +
                `RETURN DISTINCT a.username AS username, a.phoneNumber AS phoneNumber, a.pushToken AS pushToken, ` +
                `a.fullName AS fullName, a.userType AS userType, a.shopName AS shopName, a.profilePicUrl AS profilePicUrl, ` +
...
/**
 * function to authenticate a user through appleId
 * 
 */
authServices.appleLogin = (userData, cb) => {
    var responseObj = {};
    ...
    async.waterfall([
        function idLogin(callback) {
            var query = `MATCH (a : User) WHERE (a.reject <> 1 OR NOT EXISTS(a.reject)) AND a.appleId = "` + userData.appleId + `"  ` +
                `SET a.lastLogin = ` + moment().valueOf() + ` ` + condition +
                `RETURN DISTINCT ID(a) AS userId, a.appleId AS appleId, a.googleId AS googleId, a.facebookId AS facebookId, a.username AS username, a.fullName AS fullName, a.profilePicUrl AS profilePicUrl, ` +
                `a.createdOn AS createdOn, a.pushToken AS pushToken, a.deviceId AS deviceId, a.website AS website, ` +
                `a.phoneNumber AS phoneNumber, a.facebookVerified AS facebookVerified, a.googleVerified AS googleVerified, ` +
                `a.emailVerified AS emailVerified, a.location AS location, a.countrySname AS countrySname, ` +
                `a.city AS city, a.latitude AS latitude, a.longitude AS longitude, a.password AS password, a.mqttId AS mqttId LIMIT 1; `;
            ...
            dbneo4j.cypher({
                ...
                if (e) {
                    ...
                    console.log('------------------------------not registered user', d);
                    responseObj = {
                        code: 204,
                        signInType: 4,
                        message: 'no data'
        ...
        function sendResponse(d, callback) {
            ...
            userCollection.findOne({
                ...
                if (err1) {
                    ...
                    console.log('dddddxxxxxxxxxxxxxxxx', data1);
                    ...
                    userCollection.update({
                        ...
                        (mongoError, mongoData) => {
                            if (mongoError) {
                                ...
                                responseObj.username = d[0].username;
                                responseObj.fullName = d[0].fullName;
                                responseObj.userType = d[0].userType;
                                responseObj.shopName = d[0].shopName;
                                responseObj.gstNumber = d[0].gstNumber;
                                responseObj.deviceId = d[0].deviceId;
                                responseObj.pushToken = d[0].pushToken;
                                responseObj.profilePicUrl = d[0].profilePicUrl;
                                responseObj.website = d[0].website;
                                ...
                                responseObj.facebookVerified = d[0].facebookVerified;
                                responseObj.googleVerified = d[0].googleVerified;
                                responseObj.appleVerified = d[0].appleVerified;
                                responseObj.emailVerified = d[0].emailVerified;
                                callback(null, responseObj);
                            }
                        }
...
authServices.login = (userData, cb) => {
    ...
    // process.stdout.write(userData.pushToken + '\n');
    if (userData.location) condition += `, a.location = "` + userData.location + `" `;
    if (userData.city) condition += `, a.city = "` + userData.city + `" `;
    if (userData.countrySname) condition += `, a.countrySname = "` + userData.countrySname + `" `;
    if (userData.latitude) condition += `, a.latitude = "` + userData.latitude + `" `;
    ...
    async.series([
        function getUser(callback) {
            // let query = `MATCH (a : User {username : "` + userData.username + `"}) WHERE a.reject <> 1 OR NOT EXISTS(a.reject) `
            let query1 = '';
            if (userData.userType && userData.userType == "seller")
                query1 = `MATCH (a : User) WHERE (a.reject <> 1 OR NOT EXISTS(a.reject)) AND (a.email="` + userData.username + `" OR a.username="` + userData.username + `") AND a.userType="` + userData.userType + `"` + ``;
            else
                query1 = `MATCH (a : User) WHERE (a.reject <> 1 OR NOT EXISTS(a.reject)) AND (a.email="` + userData.username + `" OR a.username="` + userData.username + `") AND (NOT EXISTS(a.userType) OR a.userType <> "seller" )` + ``;
            let query = query1 +
                `SET a.pushToken = ` + JSON.stringify(pushToken) + `, a.lastLogin = ` + moment().valueOf() + ` `
                // + `SET a.lastLogin = ` + moment().valueOf() + ` `
                +
                condition +
                ...
                `a.phoneNumber AS phoneNumber, a.facebookVerified AS facebookVerified, a.googleVerified AS googleVerified, ` +
                `a.emailVerified AS emailVerified, a.location AS location, a.countrySname AS countrySname, ` +
                `a.userType AS userType, a.shopName AS shopName, a.gstNumber AS gstNumber, a.contactName AS contactName, ` +
                `a.city AS city, a.latitude AS latitude, a.longitude AS longitude, a.password AS password, a.mqttId AS mqttId LIMIT 1; `;
            ...
            dbneo4j.cypher({
                ...
                if (e) {
                    ...
                    responseObj = {
                        code: 204,
                        message: userData && userData.userType && userData.userType == "seller" ? 'seller not found' : 'user not found'
                    };
                    callback(responseObj, null);
                } else {
                    console.log('1------------', userData.password);
                                    ...
                                    userCollection.update({
                                        ...
                                        (mongoError, mongoData) => {
                                            if (mongoError) {
                                                ...
                                                responseObj.shopName = d[0].shopName;
                                                responseObj.gstNumber = d[0].gstNumber;
                                                responseObj.contactName = d[0].contactName;
                                                responseObj.mqttId = d[0].mqttId;
                                                responseObj.facebookId = d[0].facebookId;
                                                responseObj.googleId = d[0].googleId;
                                                responseObj.fullName = d[0].fullName;
                                                ...
                                                responseObj.latitude = d[0].latitude;
                                                responseObj.longitude = d[0].longitude;
                                                // updatePushToken(userData.username, pushToken);
                                                callback(null, responseObj);
                                            }
                                        });
                                } else {
