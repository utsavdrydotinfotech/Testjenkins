...
module.exports = function (app, express) {
     ...
     * @deprecated
     */
    Router.post('/upload/web',function (req,res) {
        var fileData = base64_decode(req.body.image.split(',')[1]);
        var ImageName = moment().valueOf() + ".png";
        var responseObj = {};
        var target_path = config.installFolder + 'chat/profilePics/' + ImageName;
        fs.appendFile(target_path, fileData, 'binary', function (err) {
            if (err) {
                ...
                return res.send(responseObj).status(500);
                // cb(responseObj, null);
            } else {
                var path = {};
                path.activeUrl = `https://yelo-app.xyz/chat/profilePics/${ImageName}`;
                return res.json({
                    code: 200,
                    url: path.activeUrl
                }).status(200);
            ...
            function uploadActiveImageUrl(d, cb) {
                if (!isImageUrl(req.body.activeimage)) {
                    return new Promise((resolve, reject) => {
                        ...
                        actImage = (dt.secure_url);
                        cb(null, dt.secure_url);
                    }).catch(er => {
                        cb(responseObj, null);
                    })
    ...
    Router.get('/subCategory', (req, res) => {
        ...
        console.log('inside app api',query);
                dbneo4j.cypher({
                    query: query
                }, (err, data) => {
                    if (err) {
                        ...
                        //adding this to remove any filter that has its value set to null
                        data.forEach(function(element){
                            if(element.filter && element.filter.length == 1 && element.filter[0].filId == null){
                                element.filter = [];
                            }
                        })
                        /* data.forEach(function (element) {
        ...
        getSubCategory()
            .then((result) => {
                console.log('-============13', JSON.stringify(result));
                return res.send(result).status(result.code);
            })
            .catch((error) => {
                return res.send(error).status(error.code);
    ...
    Router.route('/fields/:subCategoryName')
        .post((req, res) => { // add fields
            ...
            return new Promise((resolve, reject) => {
                ...
                console.log('admin gets filter from here',query);
