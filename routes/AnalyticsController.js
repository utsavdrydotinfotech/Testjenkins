        ...
        async.parallel([
            function basicInsights(cb) {
                let query = `MATCH (a : User {username : "` + username + `"})-[p : POSTS]->(b : Photo {postId : ` + postId + `}) `
                    ...
                    + `OPTIONAL MATCH (viewedBy : User)-[v : impression]->(b) WITH DISTINCT COUNT(v) AS totalViews, commented, likes, b, p, a `
                    + `OPTIONAL MATCH (distinctViewers : User)-[t : impression]->(b) WITH DISTINCT distinctViewers.username AS distinctViewers, totalViews, commented, likes, b, p, a `
                    + `RETURN totalViews, distinctViewers, commented, likes, b.postId AS postId, toInt(p.postedOn) AS postedOn; `;
                ...
                dbneo4j.cypher({ query: query }, (e, d) => {
                    if (e) {
                        ...
                        let distinctViews = d.length;
                        d.forEach((val, key, arr) => {
                            val.distinctViews = distinctViews;
                        });
                        responseObj = {
                            code: 200,
                            basicInsight: {
    ...
    function yearlyInsights(username, postId, cb) {
        ...
        dbneo4j.cypher({ query: query }, function (e, d) {
            if (e) {
                ...
                // console.log("ar",arr);
                for (i = 0; i < n; i++) {
                    year.push(moment(arr[i]).format('MMM'));
                }
