...
module.exports = (app, express) => {
    ...
    Router.post('/email/me', (req, res) => {
        ...
        decodeJwt(req.body.token.trim(), (eDecoded, decoded) => {
            if (eDecoded) {
                ...
                decoded.newEmail = req.body.email;
                let token = registrationToken(decoded);
                ...
                var mailData = {
                    ...
                    subject: 'verification of e-mail address ',
                    html: 'Hello, ' + req.body.email.trim() +
                        '. Please click on the link to verify your new email address <a href="' + url + '/verify-email/' + token.trim() + '/' + randomString + '">Click here to reset verify your email address</a> '
                };
                var responseObj = {};
                async.waterfall([
                    function sendMail(callback) {
                        mailgun.messages().send(mailData, function (mailgunErr, mailgunResponse) {
                            if (mailgunErr) {
                                ...
                                console.log('=======mail gun',mailgunErr)
                                callback(responseObj, null);
                            } else {
                                console.log('mail gun response=====',mailgunResponse)
                                callback(null, true);
                            }
                        });
                    },
            ...
            if (err) {
                ...
                async.waterfall([
                    ...
                    function verifyEmailId(data, cb) {
                        var query = '';
                        if (decoded.newEmail) {
                            query = `MATCH (a : User {username : "` + username + `"}) SET a.emailVerified = 1,a.email="${decoded.newEmail}" `
                                + `RETURN a.username AS username, a.emailVerified AS emailVerified LIMIT 1; `;
                        } else {
                            query = `MATCH (a : User {username : "` + username + `"}) SET a.emailVerified = 1 `
                                + `RETURN a.username AS username, a.emailVerified AS emailVerified LIMIT 1; `;
                        }
    ...
    function registrationToken(decoded) {
        var token = jsonwebtoken.sign({
            id: decoded.id,
            newEmail: decoded.newEmail,
            name: decoded.name,
            deviceType: decoded.deviceType,
            webAccessKey: decoded.webAccessKey,
